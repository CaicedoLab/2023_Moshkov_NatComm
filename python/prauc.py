...
with open('../predictions/' + split + '/2022_01_evaluation_all_data_python.csv', 'w') as f:
	writer = csv.writer(f, lineterminator = '\n')
	writer.writerow(header)
	for prediction_file in prediction_files:
		predictions_array = pd.read_csv('../predictions/' + split + '/predictions/' + prediction_file).to_numpy()[:, 1:].astype(np.float64)
		row = []
