...
if __name__ == "__main__":
	results = {}
	for i in range(5):
		with open('../predictions/chemical_cv{}/2022_01_evaluation_all_data.csv'.format(i), 'r') as f:
			next(f)
			reader = csv.reader(f)
			for row in reader:
	...
	with open('../predictions/scaffold_mean_AUC.csv', 'w') as f:
	...
	print('median')
	with open('../predictions/scaffold_median_AUC.csv', 'w') as f:
		writer = csv.writer(f, lineterminator = '\n')
		writer.writerow(header)
		for modal in results.keys():
			auc_50 = 0
