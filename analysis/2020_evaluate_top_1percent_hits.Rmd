...
result_hit_rate %>% 
  ...
  write_csv("../results/2020-09-08-hit-rate-ranked.csv")
```
...
# prototype code to test functions 
```{r}
percentage <- 0.01
sample_assay_id <- '241_715'

response

assay_ids = colnames(response[,2:length(response)])


predictions <- read_csv(file_list[10])
prediction <- predictions %>% extract2(sample_assay_id)
# response <-  response %>% extract2(sample_assay_id)

calculate_top_rank_hit_rate(response, prediction )
...
hits_per_assay_train
 ...
 descriptor <- basename(file_list[11]) %>% str_sub(13,-5) 
